...
use kawa_lending_types::address_provider::{
    AddressResponseItem, Config, ConfigResponse, ExecuteMsg, InstantiateMsg, KawaAddressType,
    QueryMsg,
};
...
fn query_address(deps: Deps, address_type: KawaAddressType) -> StdResult<AddressResponseItem> {
...
fn query_addresses(
    deps: Deps,
    address_types: Vec<KawaAddressType>,
...
fn query_all_addresses(
    ...
    let start = start_after.map(KawaAddressTypeKey::from).map(Bound::exclusive);
