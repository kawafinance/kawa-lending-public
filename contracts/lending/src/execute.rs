...
pub fn instantiate(deps: DepsMut, msg: InstantiateMsg) -> Result<Response, ContractError> {
    ...
    if !available {
        return Err(KawaError::InstantiateParamsUnavailable {}.into());
    };
    ...
    match market_option {
        ...
        Some(mut market) => {
            ...
            if should_update_interest_rates {
                ...
                //     deps.as_ref(),
                //     &config.address_provider,
                //     vec![KawaAddressType::Incentives, KawaAddressType::RewardsCollector],
                // )?;
                // let rewards_collector_addr = &addresses[&KawaAddressType::RewardsCollector];
                // let incentives_addr = &addresses[&KawaAddressType::Incentives];
...
fn update_asset_by_emergency_owner(
    ...
    if let Some(mut market) = MARKETS.may_load(deps.storage, denom)? {
        match params.borrow_enabled {
            ...
            _ => Err(KawaError::Unauthorized {}.into()),
        }
    } else {
...
pub fn withdraw(
    ...
    let addresses = address_provider::helpers::query_contract_addrs(
        ...
        &config.address_provider,
        vec![
            KawaAddressType::Oracle,
            // KawaAddressType::Incentives,
            // KawaAddressType::RewardsCollector,
        ],
    )?;
    // let rewards_collector_addr = &addresses[&KawaAddressType::RewardsCollector];
    // let incentives_addr = &addresses[&KawaAddressType::Incentives];
    let oracle_addr = &addresses[&KawaAddressType::Oracle];
