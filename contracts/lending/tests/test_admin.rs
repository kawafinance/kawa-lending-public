...
use kawa_testing::{mock_dependencies, mock_env, mock_env_at_block_time, MockEnvParams};
use kawa_utils::error::ValidationError;
...
fn proper_initialization() {
    ...
    assert_eq!(error_res, KawaError::InstantiateParamsUnavailable {}.into());
...
fn update_asset_new_reserve_factor_accrues_interest_rate() {
    ...
    let collateral = COLLATERALS
        .load(
            deps.as_ref().storage,
            (&Addr::unchecked(KawaAddressType::RewardsCollector.to_string()), "somecoin"),
        )
        .unwrap();
...
fn update_asset_by_emergency_owner() {
    ...
    // update asset with borrow_enabled = true, should have not effect on the saved market
    {
        ...
        assert_eq!(res_err, ContractError::Kawa(KawaError::Unauthorized {}));
