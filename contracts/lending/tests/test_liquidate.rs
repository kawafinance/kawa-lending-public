...
use kawa_lending_types::{
    address_provider::KawaAddressType,
    incentives,
    lending::{Collateral, Debt, ExecuteMsg, InterestRateModel, Market},
};
use kawa_testing::{mock_env, mock_env_at_block_time, KawaMockQuerier, MockEnvParams};
use kawa_utils::math;
...
struct TestSuite {
    deps: OwnedDeps<MockStorage, MockApi, KawaMockQuerier>,
...
fn rewards_collector_collateral(deps: Deps, denom: &str) -> Collateral {
    COLLATERALS
        .load(
            deps.storage,
            (&Addr::unchecked(KawaAddressType::RewardsCollector.to_string()), denom),
        )
        .unwrap()
}
...
fn expected_messages(
    ...
    vec![
        SubMsg::new(WasmMsg::Execute {
            contract_addr: KawaAddressType::Incentives.to_string(),
        ...
        SubMsg::new(WasmMsg::Execute {
            contract_addr: KawaAddressType::Incentives.to_string(),
            msg: to_binary(&incentives::ExecuteMsg::BalanceChange {
                user_addr: Addr::unchecked(KawaAddressType::RewardsCollector.to_string()),
