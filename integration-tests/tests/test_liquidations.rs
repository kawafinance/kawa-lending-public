...
fn liquidate_collateralized_loan() {
    ...
    lending.deposit(&mut mock_env, &borrower, coin(funded_osmo, "uosmo")).unwrap();

    // check HF for borrower
    let borrower_position = lending.query_user_position(&mut mock_env, &borrower);
    ...
    lending.borrow(&mut mock_env, &borrower, "uusdc", max_borrow.u128() + 1).unwrap_err();

    // borrow max allowed amount
    lending.borrow(&mut mock_env, &borrower, "uusdc", max_borrow.u128()).unwrap();
    let borrower_position = lending.query_user_position(&mut mock_env, &borrower);
    ...
    let lending_osmo_balance_before =
        mock_env.query_balance(&lending.contract_addr, "uosmo").unwrap();
    let lending_usdc_balance_before =
        mock_env.query_balance(&lending.contract_addr, "uusdc").unwrap();
    ...
    let market_osmo_before = lending.query_market(&mut mock_env, "uosmo");
    let market_usdc_before = lending.query_market(&mut mock_env, "uusdc");
    let borrower_osmo_collateral_before =
        lending.query_user_collateral(&mut mock_env, &borrower, "uosmo");
    let borrower_usdc_debt_before = lending.query_user_debt(&mut mock_env, &borrower, "uusdc");
    let liquidator_osmo_collateral_before =
        lending.query_user_collateral(&mut mock_env, &liquidator, "uosmo");
    let borrower_position_before = lending.query_user_position(&mut mock_env, &borrower);
    ...
    // lending usdc balance is increased by repayed amount
    let lending_usdc_balance = mock_env.query_balance(&lending.contract_addr, "uusdc").unwrap();
    assert_eq!(
        lending_usdc_balance.amount,
        lending_usdc_balance_before.amount + max_amount_to_repay
    );
    // lending osmo balance is the same - we need to withdraw funds (collateral) manually
    let lending_osmo_balance = mock_env.query_balance(&lending.contract_addr, "uosmo").unwrap();
    assert_eq!(lending_osmo_balance.amount, lending_osmo_balance_before.amount);
    ...
    let market_usdc = lending.query_market(&mut mock_env, "uusdc");
    let scaled_max_amount_to_repay =
        lending.query_scaled_debt_amount(&mut mock_env, coin(max_amount_to_repay.u128(), "uusdc"));
    assert_eq!(
    ...
    let market_osmo = lending.query_market(&mut mock_env, "uosmo");
    ...
    let borrower_usdc_debt = lending.query_user_debt(&mut mock_env, &borrower, "uusdc");
    ...
    lending.withdraw(&mut mock_env, &liquidator, "uosmo", None).unwrap();
    // lending osmo balance is decreased by liquidated amount
    let lending_osmo_balance = mock_env.query_balance(&lending.contract_addr, "uosmo").unwrap();
    assert_eq!(
        lending_osmo_balance.amount,
        lending_osmo_balance_before.amount - osmo_amount_to_liquidate
    );
    ...
    let market_osmo = lending.query_market(&mut mock_env, "uosmo");
    let scaled_amount_to_liquidate = lending.query_scaled_liquidity_amount(
        &mut mock_env,
...
fn liquidate_uncollateralized_loan() {
    ...
    lending.borrow(&mut mock_env, &borrower, "uusdc", borrow_amount).unwrap();
