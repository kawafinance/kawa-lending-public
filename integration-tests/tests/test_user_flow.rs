...
use kawa_testing::integration::mock_env::{MockEnv, MockEnvBuilder, Lending};
...
fn user_flow() {
    ...
    lending.deposit(&mut mock_env, &user_1, coin(deposited_atom, "uatom")).unwrap();
    let balance = mock_env.query_balance(&user_1, "uatom").unwrap();
    assert_eq!(balance.amount.u128(), funded_atom - deposited_atom);
    let collateral = lending.query_user_collateral(&mut mock_env, &user_1, "uatom");
    assert_eq!(collateral.amount.u128(), deposited_atom);

    // user_2 deposits all usdc balance
    let deposited_usdc = funded_usdc;
    lending.deposit(&mut mock_env, &user_2, coin(deposited_usdc, "uusdc")).unwrap();
    let balance = mock_env.query_balance(&user_2, "uusdc").unwrap();
    assert_eq!(balance.amount, Uint128::zero());
    let user_2_usdc_collateral = lending.query_user_collateral(&mut mock_env, &user_2, "uusdc");
    ...
    lending.borrow(&mut mock_env, &user_1, "uusdc", borrowed_usdc).unwrap();
    let balance = mock_env.query_balance(&user_1, "uusdc").unwrap();
    assert_eq!(balance.amount.u128(), borrowed_usdc);
    let user_1_usdc_debt = lending.query_user_debt(&mut mock_env, &user_1, "uusdc");
    ...
    let debt = lending.query_user_debt(&mut mock_env, &user_1, "uusdc");
    ...
    lending.repay(&mut mock_env, &user_1, coin(repayed.u128(), "uusdc")).unwrap();
    let balance = mock_env.query_balance(&user_1, "uusdc").unwrap();
    ...
    let collateral = lending.query_user_collateral(&mut mock_env, &user_2, "uusdc");
    ...
    lending.withdraw(&mut mock_env, &user_2, "uusdc", None).unwrap();
    let balance = mock_env.query_balance(&user_2, "uusdc").unwrap();
...
fn borrow_exact_liquidity() {
    ...
    lending.deposit(&mut mock_env, &provider, coin(funded_usdc, "uusdc")).unwrap();
    let balance = mock_env.query_balance(&provider, "uusdc").unwrap();
    assert_eq!(balance.amount, Uint128::zero());
    let provider_collateral = lending.query_user_collateral(&mut mock_env, &provider, "uusdc");
    assert_eq!(provider_collateral.amount.u128(), funded_usdc);

    // borrower deposits atom
    lending.deposit(&mut mock_env, &borrower, coin(funded_atom, "uatom")).unwrap();
    let balance = mock_env.query_balance(&borrower, "uatom").unwrap();
    assert_eq!(balance.amount, Uint128::zero());
    let borrower_collateral = lending.query_user_collateral(&mut mock_env, &borrower, "uatom");
    assert_eq!(borrower_collateral.amount.u128(), funded_atom);

    // check current lending balance
    let usdc_balance = mock_env.query_balance(&lending.contract_addr, "uusdc").unwrap();
    assert_eq!(usdc_balance.amount.u128(), funded_usdc);
    let atom_balance = mock_env.query_balance(&lending.contract_addr, "uatom").unwrap();
    assert_eq!(atom_balance.amount.u128(), funded_atom);

    // check markets before borrowing full liquidity
    let usdc_market_before = lending.query_market(&mut mock_env, "uusdc");
    ...
    let atom_market_before = lending.query_market(&mut mock_env, "uatom");
    ...
    lending.borrow(&mut mock_env, &borrower, "uusdc", funded_usdc).unwrap();
    let balance = mock_env.query_balance(&borrower, "uusdc").unwrap();
    assert_eq!(balance.amount.u128(), funded_usdc);
    let borrower_debt = lending.query_user_debt(&mut mock_env, &borrower, "uusdc");
    assert_eq!(borrower_debt.amount.u128(), funded_usdc);

    // check markets after borrowing full liquidity
    let atom_market = lending.query_market(&mut mock_env, "uatom");
    ...
    let usdc_market = lending.query_market(&mut mock_env, "uusdc");
    ...
    lending.borrow(&mut mock_env, &borrower, "uusdc", 1u128).unwrap_err();
}

#[test]
fn interest_rates_after_repayment() {
    // 1. Repay exact debt
    let (mut mock_env, lending, user_1) = prepare_debt_for_repayment();
    ...
    let exact_debt_repayment_result = lending.query_market(&mut mock_env, "uusdc");
    assert_eq!(
    ...
    lending.repay(&mut mock_env, &user_1, coin(repayed, "uusdc")).unwrap();
    ...
    let result = lending.query_market(&mut mock_env, "uusdc");
...
fn prepare_debt_for_repayment() -> (MockEnv, Lending, Addr) {
    let owner = Addr::unchecked("owner");
    let mut mock_env = MockEnvBuilder::new(None, owner).build();
    ...
    lending.borrow(&mut mock_env, &user_2, "uusdc", borrowed_usdc).unwrap();
...
fn internally_tracked_balances_used_for_borrow() {
    ...
    mock_env.fund_account(&lending.contract_addr, &[coin(donated_atom, "uatom")]);
    ...
    lending.deposit(&mut mock_env, &borrower2, coin(funded_osmo, "uosmo")).unwrap();
    let res = lending.borrow(&mut mock_env, &borrower2, "uatom", donated_atom);
    assert_err(
        res,
    ...
    lending.borrow(&mut mock_env, &borrower2, "uatom", funded_atom).unwrap();

    // 5. wait 10 seconds
    let user_res = lending.query_user_collateral(&mut mock_env, &borrower, "uatom");
    ...
    let new_user_res = lending.query_user_collateral(&mut mock_env, &borrower, "uatom");
...
fn interest_rates_accured_based_on_internally_tracked_balances() {
    ...
    lending.deposit(&mut mock_env, &user_1, coin(deposited_usdc, "uusdc")).unwrap();
    let collateral = lending.query_user_collateral(&mut mock_env, &user_1, "uusdc");
    assert_eq!(collateral.amount.u128(), deposited_usdc);

    // user_2 deposits some atom
    let deposited_atom = 500_000_000u128;
    lending.deposit(&mut mock_env, &user_2, coin(deposited_atom, "uatom")).unwrap();
    let collateral = lending.query_user_collateral(&mut mock_env, &user_2, "uatom");
    assert_eq!(collateral.amount.u128(), deposited_atom);
    ...
    let borrowed_usdc = 125_000_000u128;
    lending.borrow(&mut mock_env, &user_2, "uusdc", borrowed_usdc).unwrap();
    let debt = lending.query_user_debt(&mut mock_env, &user_2, "uusdc");
    ...
    lending.repay(&mut mock_env, &user_2, coin(repayed_usdc, "uusdc")).unwrap();
    let debt = lending.query_user_debt(&mut mock_env, &user_2, "uusdc");
    ...
    lending.withdraw(&mut mock_env, &user_2, "uatom", Some(Uint128::new(withdrawn_atom))).unwrap();
    let collateral = lending.query_user_collateral(&mut mock_env, &user_2, "uatom");
    ...
    lending.withdraw(&mut mock_env, &user_2, "uusdc", None).unwrap();
    let debt = lending.query_user_debt(&mut mock_env, &user_2, "uusdc");
    lending.repay(&mut mock_env, &user_2, coin(debt.amount.u128(), "uusdc")).unwrap();
