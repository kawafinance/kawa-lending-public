...
pub const ORACLE_CONTRACT_NAME: &str = "kawa-oracle-wasm";

pub struct WasmOracleTestRobot<'a> {
    runner: &'a TestRunner<'a>,
    pub astroport_contracts: AstroportContracts,
    pub kawa_oracle_contract_addr: String,
}
...
impl<'a> WasmOracleTestRobot<'a> {
    pub fn new(
        ...
        Self {
            runner,
            astroport_contracts,
            kawa_oracle_contract_addr: contract_addr,
        }
    }
    ...
    pub fn upload_and_init_contracts(
        ...
        // Instantiate Kawa Oracle Wasm contract
    ...
    pub fn set_price_source(
        ...
        let msg = kawa_lending_types::oracle::msg::ExecuteMsg::<_, Empty>::SetPriceSource {
            denom: denom.to_string(),
            price_source,
        };
        self.wasm().execute(&self.kawa_oracle_contract_addr, &msg, &[], signer).unwrap();
        self
    }
    ...
    pub fn remove_price_source(&self, signer: &SigningAccount, denom: &str) -> &Self {
        let msg = kawa_lending_types::oracle::msg::ExecuteMsg::<Empty>::RemovePriceSource {
            denom: denom.to_string(),
        };
        self.wasm().execute(&self.kawa_oracle_contract_addr, &msg, &[], signer).unwrap();
        self
    ...
    pub fn query_price_sources(
        ...
        limit: Option<u32>,
    ) -> Vec<kawa_lending_types::oracle::PriceSourceResponse<WasmPriceSourceUnchecked>> {
        let msg = &kawa_lending_types::oracle::msg::QueryMsg::PriceSources {
            start_after,
            limit,
        };
        self.wasm().query(&self.kawa_oracle_contract_addr, &msg).unwrap()
    }

    pub fn query_price_source(
        &self,
        denom: &str,
    ) -> kawa_lending_types::oracle::PriceSourceResponse<WasmPriceSourceUnchecked> {
        let msg = &kawa_lending_types::oracle::msg::QueryMsg::PriceSource {
            denom: denom.to_string(),
        };
        self.wasm().query(&self.kawa_oracle_contract_addr, &msg).unwrap()
    }

    pub fn query_price(&self, denom: &str) -> kawa_lending_types::oracle::PriceResponse {
        let msg = &kawa_lending_types::oracle::msg::QueryMsg::Price {
            denom: denom.to_string(),
        };
        self.wasm().query(&self.kawa_oracle_contract_addr, &msg).unwrap()
    }
    ...
    pub fn query_prices(
        ...
        start_after: Option<String>,
        limit: Option<u32>,
    ) -> Vec<kawa_lending_types::oracle::PriceResponse> {
        let msg = &kawa_lending_types::oracle::msg::QueryMsg::Prices {
            start_after,
            limit,
        };
        self.wasm().query(&self.kawa_oracle_contract_addr, &msg).unwrap()
    ...
    pub fn record_twap_snapshots(&self, denoms: &[&str], signer: &SigningAccount) -> &Self {
        let msg = &kawa_lending_types::oracle::msg::ExecuteMsg::<Empty, WasmOracleCustomExecuteMsg>::Custom(
    ...
    pub fn owner_update(&self, update_msg: OwnerUpdate, signer: &SigningAccount) -> &Self {
        let msg = &kawa_lending_types::oracle::msg::ExecuteMsg::<Empty>::UpdateOwner(update_msg);
        self.wasm().execute(&self.kawa_oracle_contract_addr, &msg, &[], signer).unwrap();
        self
    }

    pub fn query_config(&self) -> kawa_lending_types::oracle::ConfigResponse {
        let msg = &kawa_lending_types::oracle::msg::QueryMsg::Config {};
        self.wasm().query(&self.kawa_oracle_contract_addr, &msg).unwrap()
    }
...
pub fn get_wasm_oracle_contract(runner: &TestRunner) -> ContractType {
    match runner {
        ...
        TestRunner::MultiTest(_) => ContractType::MultiTestContract(Box::new(
            ...
            .with_migrate(kawa_oracle_wasm::contract::entry::migrate),
        )),
